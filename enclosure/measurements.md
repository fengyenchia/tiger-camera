...
| A-08 | 兩個 USB-C：各自開口寬 × 高 | 實物開口 `9.0 × 3.0`；測試片 `9.2 × 3.2`／`9.3 × 3.3`／`9.4 × 3.4` |  |  | 待測試片 |
| A-09 | 兩個 USB-C：各自中心 `X, Y` 與端口間距 | **底面、靠前蓋／ESP32 所在側**；第一個 USB：`9.5, 3.0`；第二個 USB：`20.5, 3.0`（開口皆與 ESP32 PCB 底邊貼合，不可置中） |  |  | 待量 |
| A-10 | USB-C 插頭插入方向需要的外部淨空 | `USB-C 插頭長寬：6.0 x 12.0` |  |  | 待量 |
...
| ID | 要量的項目 | 暫定值 | 實測／工程圖數值 | 來源 | 狀態 |
| --- | --- | --- | --- | --- | --- |
| B-01 | PCB 長 × 寬 × 厚 | `46.5 × 30.0 × 4.0` |  |  | 待量 |
| B-02 | 含排針／焊線的最大總深度 | `22.0` |  |  | 暫定 |
| B-03 | 可視區寬 × 高 | `30.0 × 30.0` |  |  | 暫定 |
| B-04 | 可視區相對 PCB 左／下邊偏移 | `可視區左下角距離PCB左下角X,Y：1.0, 10.5` |  |  | 待量 |
...
| C-05 | TP4056 USB-C 開孔寬 × 高、中心位置 | `寬 × 高：7.5 × 3.0`；右側面、緊鄰 KCD1-11、**直式方向**；`中心位置：7.5 向外凸出 1.0` |  |  | 待量 |
...
| C-11 | KCD1-11 面板法蘭寬 × 高 | `16.5 x 11.0` |  |  | 待量 |
| C-12 | KCD1-11 真正開孔寬 × 高、適用板厚 | 實物開口 `16.0 × 9.0`；測試片 `16.2 × 9.2`／`16.3 × 9.3`／`16.4 × 9.4` |  |  | 待測試片 |
...
| D-03 | 快門後方本體直徑、深度、端子＋焊線深度 | `直徑：12.0；深度：23.0`；`端子＋焊線深度：20.0` |  |  | 待量 |
...
| E-05 | 列印機孔徑測試：圓孔／矩形孔各自補償值 | USB-C：`9.2 × 3.2`／`9.3 × 3.3`／`9.4 × 3.4`；KCD1-11：`16.2 × 9.2`／`16.3 × 9.3`／`16.4 × 9.4` | 先印測試片，選出插拔與固定都合適的尺寸 | 待測試片 |
...
| E-09 | 第一版前後蓋定位原型外徑（寬 × 高 × 厚） | `90.0 × 125.0 × 50.0`；壁厚 `2.5`；內部包絡約 `85.0 × 120.0 × 45.0` | 保守原型尺寸，先驗證佈局與孔位，非最終尺寸 | 已決定 |
