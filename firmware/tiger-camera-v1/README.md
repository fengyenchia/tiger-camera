...
- 預覽與拍照固定使用 XGA 1024 × 768 JPEG，不在按快門時切換 sensor mode。
  VGA 預覽切換至 UXGA 1600 × 1200 拍照的兩張與六張過渡幀版本都嚴重
  欠曝，因此已棄用。XGA 比原本 VGA 多 2.56 倍像素，尚待實機確認曝光、
  預覽速度與穩定性。
...
- 固定 XGA 原圖在曝光補償 `+1` 時仍偏暗，且暗部出現紅、綠、藍色彩
  顆粒。候選調校改用曝光補償 `+2`、gain ceiling `4×`、對比 `+1` 與
  飽和度 `+1`，優先以較長曝光取代高增益；代價是移動物體可能更容易模糊。
- 網站顯示的未後製原始 JPEG 已確認仍偏暗，且對比、飽和度高於預期；TFT
  較好看是面板背光與顯色差異，不能當輸出基準。現行候選版再將亮度改為
  `+1`，對比與飽和度回到 `0`；曝光 `+2` 與 gain ceiling `4×` 不變。
- XGA 正式 JPEG quality 為 8；Serial 會輸出實際寬、高與 byte 數。
...
Gate H1 通過時，相機的預覽與拍照都維持 VGA，已證實曝光、白平衡與色彩
一致。為改善網站成品放大後的清晰度，曾測試 VGA 預覽、UXGA 1600 × 1200
拍照。第一次實測只丟棄兩張過渡幀，成功拍攝並上傳 123,060-byte JPEG，
但從 R2 取回該原始檔後確認畫面嚴重欠曝，並非 Canvas 後製造成。提高至
六張過渡幀後仍然很暗，只多看見少量影像，因此判定按快門切換解析度不適合
作為 V1 路徑。

目前候選版改為預覽與拍照皆固定 XGA 1024 × 768，不再於快門時切換 sensor
mode。XGA 像素數為 VGA 的 2.56 倍，且可讓自動曝光與白平衡持續收斂；
這項變更仍必須重新測試預覽速度、原圖曝光、連拍與上傳穩定性。

固定 XGA 第一次實測已能看見正常內容，但曝光補償 `+1` 的網站原圖仍偏暗，
TFT 與原圖也可見彩色顆粒。候選版改為曝光補償 `+2`，並將 gain ceiling
由 `8×` 降至 `4×`、對比與飽和度各降一級，避免高增益與強色彩把噪點
放大。TFT 預覽原先把 XGA 先解碼成 128 × 96 再放大至面板，會明顯變糊；
現在改為先解碼成 256 × 192，再縮小至 128 × 160。清晰度、更新速度、
曝光、噪點與動態模糊必須一起實測。

128 × 160 螢幕只能用來取景，不能單獨判定 XGA 原圖是否真的失焦。若後續
