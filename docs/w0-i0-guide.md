...
- 2026-08-24：Android Chrome 的 W0／I0 功能流程由使用者確認通過。
- 目前可進入 P0 鋰電池與 5V 升壓模組。
- iPhone Safari 與 NFC 重複 10 次的量化紀錄改列 R0 發布前檢查，不冒充已測結果。
...
| 項目 | iPhone Safari（R0） | Android Chrome |
| --- | --- | --- |
| 開啟 `/create` | [ ] | [x] |
| 6 位碼領取 | [ ] | [x] |
| 原圖與 Canvas 預覽 | [ ] | [x] |
...
- W0：Android Chrome 完成領取、後製、下載、公開、相簿與權限流程，沒有阻塞性錯誤。
- I0：Android 能開啟固定網址，領取碼流程不依賴 NFC 動態寫入。
- 失敗案例、裝置與瀏覽器版本已記入測試紀錄。
- Android 功能 Gate 通過後更新 `PROJECT_STATUS.md` 並開始 P0；NFC 與外殼位置仍應等重複掃描與實體配置後再永久鎖定。
