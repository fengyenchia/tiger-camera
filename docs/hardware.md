...
| 快門按鍵 | 短按拍照 |
| NTAG213／NTAG215 貼紙 | 被動保存最新照片固定網址；相機端不需要 NFC 電子模組 |
...
使用者提供的蝦皮頁有 1.8 吋與 1.44 吋等多個選項：

- 1.44 吋通常為 ST7735S、128 × 128、SPI，體積較小，成本版 V1 優先。
- 1.8 吋通常為 ST7735S、128 × 160、SPI，可視面積較大，但 PCB 與耗電通常也較大。
- 商品標題把兩種尺寸共寫成 128 × 160，不能據此假定 1.44 吋也是 128 × 160。
- 優先選不含額外 microSD 卡槽的 8-pin 模組，避免重複硬體與 GPIO 混淆。
- 下單前確認控制器、解析度、供電、邏輯電壓、腳位順序與 PCB 尺寸。
...
4. 螢幕與 `/latest.jpg` 都讀取這份受 mutex 保護的 buffer。
5. 下一張成功照片取代上一張；關機後照片消失。

代價是沒有**裝置端**永久保存、裝置端 JSON metadata、多照片相簿與重開機復原；這不影響手機把原圖與後製圖永久上傳到私人雲端相簿。若需要斷網、關機後仍能從相機復原照片，再購買 microSD 並新增第二個決策 Gate；不在成本版第一批採購。
...
- 手機先連上相機 Wi‑Fi 後，感應 NFC 可開啟最新照片。
