...
1. **E0**：先製作外徑 `90 × 125 × 50 mm`、壁厚 `2.5 mm` 的直式前後蓋定位原型；鏡頭朝外、螢幕朝使用者。頂面快門、兩個 ESP32 USB-C 由底面外露、TP4056 僅保留充電孔且不設 LED 觀察孔、MT3608 電位器封在殼內。鏡頭孔第一版採 `Ø9.0 mm`；USB-C／KCD1-11 先以 `+0.2–0.4 mm` 三種矩形孔測試片確認。電池艙內淨空至少 `48 × 33 × 10 mm`，不壓緊電池。定位原型通過後才加入 M2 柱位並縮小外形。
