...
1. **E0**：Onshape 第一版直式前後蓋定位原型目前為外徑 `90 × 105 × 50 mm`、壁厚 `2.5 mm`，內部包絡約 `85 × 100 × 45 mm`；鏡頭朝外、螢幕朝使用者。頂面快門；ESP32 的兩個 USB-C 不做外殼開孔，僅在拆開後蓋時供燒錄與維修使用，因此 ESP32 可上移；右側只保留 TP4056 充電孔與 KCD1-11 開關孔，TP4056 不設 LED 觀察孔，MT3608 電位器封在殼內。鏡頭孔第一版採 `Ø9.0 mm`；TP4056／KCD1-11 先以 `+0.2–0.4 mm` 三種矩形孔測試片確認。電池艙內淨空至少 `48 × 33 × 10 mm`，不壓緊電池。高度暫維持 `105 mm`；完成實裝排線後再評估縮到約 `95 mm`，並在確認模組平面排列後評估約 `80 mm` 外寬。
